...
class CelebADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        l_image = torch.tensor(l_image, dtype=torch.float32).unsqueeze(0) / 255.0  # Normalize L channel
        ab_image = torch.tensor(ab_image, dtype=torch.float32).permute(2, 0, 1) / 127.5 - 1.0  # Normalize AB channels
